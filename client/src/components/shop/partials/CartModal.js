...
import { quantity, subTotal, totalCost } from "./Mixins";
  ...
  const fetchData = async () => {
    try {
      ...
      if (responseData && responseData.Products) {
        ...
        console.log("Cart ", responseData.Products)
      }
    } catch (error) {
      console.log(error);
    }
            ...
            {data.cartTotalCost ? (
              <Fragment>
                {isAuthenticate() ? (
                  ...
                  <div
                    className="px-4 py-2 bg-white text-black text-center cursor-pointer"
                    onClick={(e) => {
                      history.push("/");
                      cartModalOpen();
                      dispatch({
              ...
              <div className="px-4 py-2 bg-white text-black text-center cursor-not-allowed">
                Checkout
              </div>
            )}
          </div>
